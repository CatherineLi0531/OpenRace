...
struct SharedMemory {
  ...
  std::map<ObjID, std::map<ThreadID, std::vector<const FreeEvent *>>> objFrees;
 ...
 public:
  ...
  [[nodiscard]] std::map<ThreadID, std::vector<const FreeEvent *>> getThreadedFrees(const pta::ObjTy *obj) const;
};
}  // namespace race