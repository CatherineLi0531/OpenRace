...
#include "InsertFakeCallForDefaultCudaStream.h"
...
// We need to insert a fake forkStreamCall (ex. cudaStreamCreate(0)) to handle the default stream of each host thread
// Should be modeled similarly to fakecallforguardblocks? (inserted below)
// How do we know this is a cuda file? Maybe use cudaSetupArgument / cudaConfigureCall?
// Cuda Streams are not ordered with numbers in CUDA specification. These are created as cudaStream_t vars,
// and then referenced in calls later by name, so alternatively create a fake variable initialization
