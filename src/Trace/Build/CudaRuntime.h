...
class CudaRuntime : public Runtime {
  ...
  struct UnjoinedKernel {
    ...
    std::vector<CudaBlockFork> blocks;
    std::vector<CudaWarpFork> warps;
    std::vector<CudaThreadFork> threads;

    UnjoinedKernel(const ForkEvent *forkEvent, std::shared_ptr<const CudaGridFork> forkIR)
        : forkEvent(forkEvent), forkIR(forkIR) {}

    int getNumberOfThreads(){ return blocks.size() * warps.size() * threads.size(); }
  };
  
  // Mapping of unjoined Cuda kernels
  std::map<CudaGridFork, UnjoinedKernel> unjoinedKernels;
